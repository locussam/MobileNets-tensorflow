...
def get_mobilenet(optimizer, weight_decay=None, image_size=224, num_classes=1001):
    """Create a MobileNet computational graph. Width multiplier alpha = 1.0
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('preprocessing'):
            ...
            mean = tf.constant([0.5, 0.5, 0.5], tf.float32, [3])
            inverse_std = tf.constant([2.0, 2.0, 2.0], tf.float32, [3])
            X /= f255
            X -= mean
            X *= inverse_std
